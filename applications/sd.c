...
{
	rt_kprintf("DS_ADDR is 0x%x\r\n", base->DS_ADDR);
	rt_kprintf("BLK_ATT is 0x%x\r\n", base->BLK_ATT);
	rt_kprintf("CMD_ARG is 0x%x\r\n", base->CMD_ARG);
	rt_kprintf("CMD_XFR_TYP is 0x%x\r\n", base->CMD_XFR_TYP);
	rt_kprintf("CMD_RSP0 is 0x%x\r\n", base->CMD_RSP0);
	rt_kprintf("CMD_RSP1 is 0x%x\r\n", base->CMD_RSP1);
	rt_kprintf("CMD_RSP2 is 0x%x\r\n", base->CMD_RSP2);
	rt_kprintf("CMD_RSP3 is 0x%x\r\n", base->CMD_RSP3);
	rt_kprintf("DATA_BUFF_ACC_PORT is 0x%x\r\n", base->DATA_BUFF_ACC_PORT);
	rt_kprintf("PRES_STATE is 0x%x\r\n", base->PRES_STATE);
	rt_kprintf("PROT_CTRL is 0x%x\r\n", base->PROT_CTRL);
	rt_kprintf("SYS_CTRL is 0x%x\r\n", base->SYS_CTRL);
	rt_kprintf("INT_STATUS is 0x%x\r\n", base->INT_STATUS);
	rt_kprintf("INT_STATUS_EN is 0x%x\r\n", base->INT_STATUS_EN);
	rt_kprintf("INT_SIGNAL_EN is 0x%x\r\n", base->INT_SIGNAL_EN);
	rt_kprintf("AUTOCMD12_ERR_STATUS is 0x%x\r\n", base->AUTOCMD12_ERR_STATUS);
	rt_kprintf("HOST_CTRL_CAP is 0x%x\r\n", base->HOST_CTRL_CAP);
	rt_kprintf("WTMK_LVL is 0x%x\r\n", base->WTMK_LVL);
	rt_kprintf("MIX_CTRL is 0x%x\r\n", base->MIX_CTRL);	
	rt_kprintf("FORCE_EVENT is 0x%x\r\n", base->FORCE_EVENT);
	rt_kprintf("ADMA_ERR_STATUS is 0x%x\r\n", base->ADMA_ERR_STATUS);
	rt_kprintf("ADMA_SYS_ADDR is 0x%x\r\n", base->ADMA_SYS_ADDR);
	rt_kprintf("DLL_CTRL is 0x%x\r\n", base->DLL_CTRL);
	rt_kprintf("DLL_STATUS is 0x%x\r\n", base->DLL_STATUS);
	rt_kprintf("CLK_TUNE_CTRL_STATUS is 0x%x\r\n", base->CLK_TUNE_CTRL_STATUS);
	rt_kprintf("VEND_SPEC is 0x%x\r\n", base->VEND_SPEC);
	rt_kprintf("MMC_BOOT is 0x%x\r\n", base->MMC_BOOT);
	rt_kprintf("VEND_SPEC2 is 0x%x\r\n", base->VEND_SPEC2);
	rt_kprintf("TUNING_CTRL is 0x%x\r\n", base->TUNING_CTRL);
...
void CardInsertDetect(USDHC_Type *base)
{
	if ((base->PRES_STATE & USDHC_PRES_STATE_CINST_MASK) == 0) {
		rt_kprintf("please insert sd card\r\n");
	}

	while(!((base->PRES_STATE >> USDHC_PRES_STATE_CINST_SHIFT) & 0x1));
	rt_kprintf("sd card has been inserted\r\n");
}
...
static int SD_SelectBusTiming(USDHC_Type *base)
{
	int err;
	u32 switch_func[64];
	u32 arg = 0x80fffff1;
	...
	if ((switch_func[13] >> 16) && 0) {
		rt_kprintf("the card can not supprt high speed\r\n");
		return -1;
	}
	rt_kprintf("the card supported mode is 0x%x\r\n", switch_func[3] >> 16);
	...
	if((switch_func[4] >> 24) == 1)
		rt_kprintf("set card high speed success\r\n", switch_func[3] >> 16);
...
int sd_init(USDHC_Type *base)
{
	...
	for (retries = 0; retries < 10; retries++) {
		...
		if (err == 0)
		{
			rt_kprintf("rspesp = [0x%x 0x%x 0x%x 0x%x]\n", base->CMD_RSP0, base->CMD_RSP1, base->CMD_RSP2, base->CMD_RSP3);
			break;
		}
	}
	...
	for(retries = 0; retries < 5000; retries++) {
		...
		if (base->CMD_RSP0 & kSD_OcrPowerUpBusyFlag) {
			...
			rt_kprintf("ocr is 0x%x\r\n", g_sd_card.ocr);
			break;
		}
	}

	if (retries >= 1000 ) {
		rt_kprintf("HandShakeOperationConditionFailed\r\n");
		return -1;
	}

	/* check 1.8V support */
	if (g_sd_card.ocr & kSD_OcrSwitch18AcceptFlag) {
		rt_kprintf("support 1.8v\r\n");
	}
	...
	rt_kprintf("relative address is 0x%x\r\n", g_sd_card.relativeAddress);
	...
	rt_kprintf("card is %s", g_sd_card.csd.csdStructure ? "SDHC/SDXC" : "SDSC");
	rt_kprintf("card block count is %d\r\n", g_sd_card.blockCount);
	rt_kprintf("card sector size is %d\r\n", g_sd_card.blockSize);
	rt_kprintf("card command class is 0x%x\r\n", g_sd_card.csd.cardCommandClass);
	...
	rt_kprintf("scr[0] is 0x%x, scr[1] is 0x%x\r\n", raw_scr[0], raw_scr[1]);
	rt_kprintf("sd specification is 0x%x\r\n", g_sd_card.scr.sdSpecification);
	if ((g_sd_card.scr.sdBusWidths & 0x4U) == 0) {
		rt_kprintf("The card can not support 4bit width");
		return -1;
	}
	/* speed class control cmd */
	if ((g_sd_card.scr.commandSupport & 0x01U) == 0)
	{
		rt_kprintf("The card can not support Speed Class Control (CMD20)\r\n");
	}
	/* set block count cmd */
	if ((g_sd_card.scr.commandSupport & 0x02U) == 0)
	{
		rt_kprintf("The card can not support Support SetBlockCountCmd (CMD23)\r\n");
	}

	/* Set to max frequency in non-high speed mode. */
	g_sd_card.busClock_Hz = USDHC_SetSdClock(base, 198000000U, SD_CLOCK_25MHZ);

#if 0 //liu-2021å¹´5æœˆ15æ—¥14:43:54
	...
	//rt_kprintf("clock is %d, base->SYS_CTRL is 0x%x\r\n", g_sd_card.busClock_Hz, base->SYS_CTRL);
	rt_kprintf("sd init sccessful\r\n");
...
static int MMC_SetExtendedCsdConfig(USDHC_Type *base, mmc_extended_csd_access_mode_t access_mode, uint8_t index, uint8_t value)
{
	int err;
...
static int MMC_SetBusWidth(USDHC_Type *base, int width)
{
	...
	err = SD_WaitReadWriteComplete(base);
	if (err & (1 << 7)) {
		rt_kprintf("failed switch to bus width %d\r\n", width);
		return -1;
	}
...
static int MMC_SwitchBusMode(USDHC_Type *base, mmc_high_speed_timing_t mode)
{
	int err;
	...
	err = SD_WaitReadWriteComplete(base);
	if (err & (1 << 7)) {
		rt_kprintf("failed switch bus mode\r\n");
		return -1;
	}
...
static int MMC_SelectPartition(USDHC_Type *base, mmc_extended_csd_access_mode_t access_mode, mmc_access_partition_t part_number)
{
	int err;
	...
	err = SD_WaitReadWriteComplete(base);
	if (err & (1 << 7)) {
		rt_kprintf("failed select partition\r\n");
		return -1;
	}
...
int mmc_init(USDHC_Type *base)
{
	...
	rt_kprintf("csd tran speed is 0x%x\r\n", g_mmc_card.csd.transferSpeed);
	...
	rt_kprintf("g_mmc_card.extendedCsd.sectorCount is %d\r\n", g_mmc_card.extendedCsd.sectorCount);
