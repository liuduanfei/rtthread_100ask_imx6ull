...
static void sdio_irq_thread(void *param)
{
    ...
    while (1)
    {
        if (rt_sem_take(host->sdio_irq_sem, RT_WAITING_FOREVER) == RT_EOK)
        {
            ...
            if (ret)
            {
                LOG_D("error %d reading SDIO_REG_CCCR_INT_PEND", ret);
                goto out;
            }
            ...
            for (i = 1; i <= 7; i++)
            {
                if (pending & (1 << i))
                {
                    ...
                    if (!func)
                    {
                        LOG_D("pending IRQ for "
                            "non-existant function %d", func->num);
                        goto out;
                    }
                    ...
                    {
                        LOG_D("pending IRQ with no register handler");
                        goto out;
                    }
                }
            }
                           ...
                           rt_sdio_irq_handler_t   *handler)
{
    ...
    LOG_D("SDIO: enabling IRQ for function %d", func->num);

    if (func->irq_handler)
    {
        LOG_D("SDIO: IRQ for already in use.");
...
rt_int32_t sdio_detach_irq(struct rt_sdio_function *func)
{
    ...
    LOG_D("SDIO: disabling IRQ for function %d", func->num);
...
rt_int32_t sdio_enable_func(struct rt_sdio_function *func)
{
    ...
    LOG_D("SDIO: enabling function %d\n", func->num);
    ...
    LOG_D("SDIO: enabled function successfull");

    return 0;

err:
    LOG_D("SDIO: failed to enable function %d", func->num);
    return ret;
}
...
rt_int32_t sdio_disable_func(struct rt_sdio_function *func)
{
    ...
    LOG_D("SDIO: disabling function %d", func->num);
    ...
    LOG_D("SDIO: disabled function successfull");

    return 0;

err:
    LOG_D("SDIO: failed to disable function %d", func->num);
    return -RT_EIO;
}
